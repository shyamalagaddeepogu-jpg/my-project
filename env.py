...
class DeliveryDispatcherEnv:
    def __init__(self, task_id: str = "task_easy"):
        ...
        self.driver_usage:           dict[str, int] = {}   # per-driver delivery count
        self.time_margins:           list[float]    = []   # (window - est_time) per on-time delivery
        self.time_margin_ratios:     list[float]    = []   # margin/window ratio per on-time delivery
        self.time_windows_at_assign: list[float]    = []   # window value at assignment moment
    ...
    def _apply_action(self, action: Action):
        ...
        if action.action_type == "assign":
            if not action.driver_id or not action.order_id:
                return 0.0, "Invalid assign — missing driver_id or order_id", partial

            driver = self.drivers.get(action.driver_id)
            order  = self.orders.get(action.order_id)

            if driver is None:
                return 0.0, f"Driver {action.driver_id} not found", partial
            if order is None:
                return 0.0, f"Order {action.order_id} not found", partial
            if order.status != OrderStatus.PENDING:
                return 0.0, f"Order {action.order_id} is not pending (status={order.status})", partial
            if not driver.available:
                return 0.0, f"Driver {action.driver_id} is not available", partial
            ...
            est_time = travel_dist * 0.5 * traffic   # base: 0.5 min/unit, scaled by traffic
            ...
            if sla_ok:
                ...
                reward = 1.0 if order.priority == Priority.HIGH else 0.8
                partial["on_time"]     = True
                partial["time_margin"] = round(time_margin, 2)
                return min(1.0, reward), f"Order {order.order_id} delivered on time", partial
                ...
                return 0.3, f"Order {order.order_id} delivered late (traffic={traffic:.1f}x)", partial

        elif action.action_type == "cancel":
            order = self.orders.get(action.order_id)
            if order is None or order.status != OrderStatus.PENDING:
                return 0.0, "Cannot cancel — order not found or not pending", partial
            order.status = OrderStatus.CANCELLED
            self.failed += 1
            return 0.0, f"Order {order.order_id} cancelled by agent", partial

        elif action.action_type == "delay":
            order = self.orders.get(action.order_id)
            if order is None or order.status != OrderStatus.PENDING:
                return 0.0, "Cannot delay — order not found or not pending", partial
            order.time_window += 10
            return 0.1, f"Order {order.order_id} time window extended by 10 min", partial

        else:
            return 0.0, f"Unknown action type: {action.action_type}", partial
    ...
    def _check_done(self) -> bool:
        if self.step_count >= self.config.max_steps:
            return True
        # only PENDING counts as "still active" — IN_TRANSIT resolves instantly
        # so we only need to watch for remaining PENDING orders
        pending = [
    ...
    def _get_observation(self) -> Observation:
        pending   = [o for o in self.orders.values() if o.status == OrderStatus.PENDING]
        # only expose drivers with remaining effective capacity
        available = [
    ...
    def _episode_stats(self) -> dict:
        ...
        return {
            ...
            "step_count":             self.step_count,
            # keys consumed by tasks.py bonus functions
